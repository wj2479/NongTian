...
public class ImageUtil {
     ...
     * @return
     */
    public static Bitmap drawTextToLeftBottom(Context context, Bitmap bitmap, List<String> labels, int size, int color, int paddingLeft, int paddingBottom, int lineMargin) {
        Paint paint = new Paint(Paint.ANTI_ALIAS_FLAG);
        paint.setColor(color);
        paint.setTextSize(dp2px(context, size));
        Rect bounds = new Rect();
        return drawTextToBitmap(context, bitmap, labels, paint, bounds,
                dp2px(context, paddingLeft),
                bitmap.getHeight() - (dp2px(context, size) + dp2px(context, lineMargin)) * (labels.size() - 1) - dp2px(context, paddingBottom), lineMargin);
    }
    ...
    //图片上绘制文字
    private static Bitmap drawTextToBitmap(Context context, Bitmap bitmap, List<String> labels, Paint paint, Rect bounds, int paddingLeft, int paddingTop, int lineMargin) {
        android.graphics.Bitmap.Config bitmapConfig = bitmap.getConfig();
        ...
        for (String label : labels) {
            canvas.drawText(label, paddingLeft, top, paint);
            top += paint.getTextSize() + dp2px(context, lineMargin);
        }
        return bitmap;
    }
